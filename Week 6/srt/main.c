...
void* seppic(void *arg1)
{
    ...
    for( int px = *rownum; px<width; px = px + nthreads )
    {
...
//    
//    int *newdim = (int*) malloc(sizeof(int*) * nthreads);
...
//    {
//        int check = pthread_create(&TID[i], NULL, seppic, &newdim[i]);
//        if (check != 0)
//        {
//            fprintf(stderr, "Allocate memory wrong.\n");
//            exit(-1);
//        }
//    }
    int alltid[nthreads];
    pthread_t allt[nthreads];
    
    for (int i = 0; i < nthreads; i++)
    {
        alltid[i] = i;
        int check = pthread_create(&allt[i], NULL, seppic, &alltid[i]);
        if (check != 0)
        {
            fprintf(stderr, "Allocate memory wrong.\n");
            exit(-1);
...
//        pthread_join(TID[j], NULL);
    ...
    for(int m = 0; m < width; m++)
    {
        ...
        {
            printf( "%.0f %.0f %.0f\n",
		    scaled_color[m][n][0], scaled_color[m][n][1], scaled_color[m][n][2] );
        }
        printf( "\n" );
    }
